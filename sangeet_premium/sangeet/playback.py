from flask import Blueprint , session , jsonify , send_file  , url_for , render_template , request , redirect , render_template_string , make_response
...
def api_search():
    ...
    For regular queries, it concurrently queries both:
      - yt-dlp (using the ytsearch syntax), and
      - YTMusic (via util.search_songs)
    and merges the results, removing duplicates.
    """
    ...
    # === 3. Process empty query: Return a default set of songs ===
    if not q:
        ...
        for song in local_songs.values():
            if song["id"] not in seen_ids:
                combined_local.append(song)
                seen_ids.add(song["id"])

        # Then add a few songs from a set of categories using YTMusic.
        for c in cats:
            for song in util.search_songs(c):
                if song["id"] not in seen_ids:
        ...
        full_list = combined_local + combined
        start = page * limit
        end = start + limit
        return jsonify(full_list[start:end])
...
def get_insights():
    ...
    try:
        insights = {
            "overview": util.get_overview_stats(c),
            "recent_activity": util.get_recent_activity(c),
            "top_artists": util.get_top_artists(c),
            "listening_patterns": util.get_listening_patterns(c),
            "completion_rates": util.get_completion_rates(c)
        }
...
def api_listen_end():
    ...
    try:
        ...
        # Duration and listened_duration can be None/null
...
@login_required
def api_queue():
    """Return last 50 songs from user's history."""
    user_id = session['user_id']
    return jsonify(util.get_play_history(user_id, limit=50))
...
def stream_file(song_id):
    ...
    try:
        ...
        if not range_header:
            resp = make_response(send_file(flac_path, mimetype="audio/flac"))
...
def api_stream_local(song_id):
    ...
    try:
        ...
        if not range_header:
            return send_file(path, mimetype="audio/flac", as_attachment=False)
